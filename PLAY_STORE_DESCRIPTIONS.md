# Play Store Açıklamaları / Store Listings

## Türkçe (tr-TR)

### Kısa Açıklama (80 karakter limiti)
```
ZeitLog - Ücretsiz iş takip uygulaması. Tek dokunuşla giriş/çıkış kaydı.
```

### Uzun Açıklama (4000 karakter limiti)
```
ZeitLog - Ücretsiz ve Reklamsız İş Takip Uygulamanız

ZeitLog, iş saatlerinizi kolayca takip etmenizi sağlayan, tamamen ücretsiz ve reklamsız bir uygulamadır. Tek dokunuşla giriş ve çıkış saatlerinizi kaydedin, çalışma sürelerinizi otomatik olarak hesaplayın.

✨ ÖZELLİKLER:

📱 Tek Dokunuşla Kayıt
• Giriş ve çıkış saatlerinizi tek dokunuşla kaydedin
• Gerçek zamanlı çalışma süresi gösterimi
• Modern ve sezgisel kullanıcı arayüzü

☁️ Bulut Yedekleme
• Firebase ile güvenli veri senkronizasyonu
• Verilerinizi asla kaybetmeyin
• Tüm cihazlarınızda senkronize

⌚ Wear OS Desteği
• Akıllı saatinizde giriş saatini görün
• Telefon uygulaması ile otomatik senkronizasyon

🌍 Çoklu Dil Desteği
• Türkçe, İngilizce ve Almanca
• Otomatik dil algılama

📊 Detaylı Raporlama
• Haftalık ve günlük çalışma süreleri
• Tatil günü takibi
• Esnek mola süresi ayarları
• CSV dışa/içe aktarma

🎨 Modern Tasarım
• Karanlık ve açık tema desteği
• Responsive tasarım (tüm ekran boyutları)
• Güzel animasyonlar ve geçişler

🔒 Gizlilik ve Güvenlik
• Verileriniz güvende
• Kullanıcı verileri izole edilmiş
• Hesap silme özelliği

💯 Tamamen Ücretsiz
• Abonelik yok
• Reklam yok
• Gizli ücret yok
• Açık kaynak kodlu

📈 KULLANIM ALANLARI:

• Çalışma saatlerini takip etmek
• İş giriş/çıkış kayıtları tutmak
• Haftalık çalışma sürelerini hesaplamak
• Tatil günlerini işaretlemek
• Çalışma verilerini yedeklemek

🎯 NEDEN ZEITLOG?

ZeitLog, iş saatlerinizi takip etmek için ihtiyacınız olan her şeyi sunar, karmaşık özellikler olmadan. Basit, hızlı ve güvenilir.

Hemen indirin ve iş saatlerinizi zahmetsizce takip etmeye başlayın!

📧 Destek: taytekofficial@gmail.com
🌐 GitHub: https://github.com/ttimocin/ZeitLog
```

---

## English (en-US)

### Short Description (80 character limit)
```
ZeitLog - Free work time tracker. One-tap check-in/out recording.
```

### Long Description (4000 character limit)
```
ZeitLog - Your Free and Ad-Free Work Time Tracking App

ZeitLog is a completely free and ad-free app that makes it easy to track your work hours. Record your check-in and check-out times with a single tap, and automatically calculate your working hours.

✨ FEATURES:

📱 One-Tap Recording
• Record your check-in and check-out times with a single tap
• Real-time working hours display
• Modern and intuitive user interface

☁️ Cloud Backup
• Secure data synchronization with Firebase
• Never lose your data
• Synchronized across all your devices

⌚ Wear OS Support
• View your entry time on your smartwatch
• Automatic synchronization with phone app

🌍 Multi-Language Support
• English, Turkish, and German
• Automatic language detection

📊 Detailed Reporting
• Weekly and daily working hours
• Holiday tracking
• Flexible break time settings
• CSV export/import

🎨 Modern Design
• Dark and light theme support
• Responsive design (all screen sizes)
• Beautiful animations and transitions

🔒 Privacy and Security
• Your data is secure
• User data is isolated
• Account deletion feature

💯 Completely Free
• No subscriptions
• No ads
• No hidden fees
• Open source

📈 USE CASES:

• Track work hours
• Keep check-in/out records
• Calculate weekly working hours
• Mark holidays
• Backup work data

🎯 WHY ZEITLOG?

ZeitLog provides everything you need to track your work hours without unnecessary complexity. Simple, fast, and reliable.

Download now and start tracking your work hours effortlessly!

📧 Support: taytekofficial@gmail.com
🌐 GitHub: https://github.com/ttimocin/ZeitLog
```

---

## Deutsch (de-DE)

### Kurzbeschreibung (80 Zeichen Limit)
```
ZeitLog - Kostenloser Arbeitszeiterfassung. Ein-Tipp Ein-/Ausstempeln.
```

### Langbeschreibung (4000 Zeichen Limit)
```
ZeitLog - Ihre kostenlose und werbefreie Arbeitszeiterfassungs-App

ZeitLog ist eine völlig kostenlose und werbefreie App, die es einfach macht, Ihre Arbeitszeiten zu erfassen. Erfassen Sie Ihre Ein- und Ausstempelzeiten mit einem einzigen Tippen und berechnen Sie automatisch Ihre Arbeitsstunden.

✨ FUNKTIONEN:

📱 Ein-Tipp Erfassung
• Erfassen Sie Ihre Ein- und Ausstempelzeiten mit einem einzigen Tippen
• Echtzeit-Anzeige der Arbeitsstunden
• Moderne und intuitive Benutzeroberfläche

☁️ Cloud-Backup
• Sichere Datensynchronisierung mit Firebase
• Verlieren Sie nie Ihre Daten
• Synchronisiert auf allen Ihren Geräten

⌚ Wear OS Unterstützung
• Sehen Sie Ihre Einstempelzeit auf Ihrer Smartwatch
• Automatische Synchronisierung mit der Telefon-App

🌍 Mehrsprachige Unterstützung
• Deutsch, Englisch und Türkisch
• Automatische Spracherkennung

📊 Detaillierte Berichte
• Wöchentliche und tägliche Arbeitsstunden
• Feiertagsverfolgung
• Flexible Pausenzeit-Einstellungen
• CSV-Export/Import

🎨 Modernes Design
• Unterstützung für dunkles und helles Theme
• Responsives Design (alle Bildschirmgrößen)
• Schöne Animationen und Übergänge

🔒 Datenschutz und Sicherheit
• Ihre Daten sind sicher
• Benutzerdaten sind isoliert
• Kontolöschfunktion

💯 Völlig kostenlos
• Keine Abonnements
• Keine Werbung
• Keine versteckten Gebühren
• Open Source

📈 ANWENDUNGSFÄLLE:

• Arbeitszeiten erfassen
• Ein-/Ausstempelaufzeichnungen führen
• Wöchentliche Arbeitsstunden berechnen
• Feiertage markieren
• Arbeitsdaten sichern

🎯 WARUM ZEITLOG?

ZeitLog bietet alles, was Sie brauchen, um Ihre Arbeitszeiten zu erfassen, ohne unnötige Komplexität. Einfach, schnell und zuverlässig.

Laden Sie jetzt herunter und beginnen Sie mühelos mit der Erfassung Ihrer Arbeitszeiten!

📧 Support: taytekofficial@gmail.com
🌐 GitHub: https://github.com/ttimocin/ZeitLog
```

---

## Play Store'da Kullanım

### Kısa Açıklama (Short Description)
- Play Console → Store presence → Main store listing
- "Short description" bölümüne yukarıdaki kısa açıklamaları kopyalayın

### Uzun Açıklama (Full Description)
- Aynı sayfada "Full description" bölümüne uzun açıklamaları kopyalayın

### Notlar:
- Her dil için ayrı ayrı ekleyin
- Emoji'ler Play Store'da desteklenir
- 4000 karakter limiti var (uzun açıklama için)
- 80 karakter limiti var (kısa açıklama için)

